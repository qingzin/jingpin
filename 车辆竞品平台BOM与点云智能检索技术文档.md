...
报告中的数据规模与效果指标采用示例填充值，便于形成完整技术报告。考虑到现阶段尚未建设人工标注查询集，评估部分只保留可直接统计或可通过系统日志获得的指标，包括车型数量、BOM 数据条数、点云文件数量和平均响应耗时，正式发布前由平台侧替换为实际统计值。
...
| --- | --- | --- | --- | --- |
| BGE-M3 | 支持中文和多语言语义表达，适合短文本、句子和较长上下文的统一向量化；本项目使用 1024 维向量接入 Qdrant 余弦检索。 | 适合处理中英混合零件名、层级路径、系统位置和工程简称，能在“语义召回+结构化过滤”架构中承担主要召回能力。 | 仍需结合内部试用反馈和后续标注数据验证语义召回效果；对零件号、尺寸、重量等精确字段不能替代结构化过滤。 | 推荐作为默认模型。 |
...
模型选型不以公开榜单指标直接作为业务结论。本文先从业务语义适配性、部署可用性和平台集成成本角度说明选择依据；后续如建设人工标注查询集，再补充同口径召回效果对比。
...
双库架构的分工比较明确。DuckDB 负责保存可审计、可过滤、可展示的结构化数据，包括来源文件、来源行、零件名称、零件号、车型、厂家、材料、工艺、尺寸、重量、层级数组、点云路径和原始字段。Qdrant 负责保存零件记录的向量表达，并使用余弦相似度完成候选召回。两者通过统一记录编号关联，避免向量库承载过多复杂业务字段，也避免结构化库承担高维向量近邻搜索压力。
...
服务接口面向竞品平台开发者提供后台能力，而不是要求最终用户直接使用本地检索端。字段枚举接口为前端筛选器提供车型、材料、工艺等可选项；检索接口接收查询词、召回数量、分页和过滤参数；自然语言检索接口尝试从一句话中提取结构化约束；下载接口根据检索结果中的点云标识返回受控文件。这样设计可以让竞品平台前端专注交互体验，后台服务统一处理检索语义、数据过滤和文件边界。
...
现阶段项目尚未建设成规模的人工标注查询集，因此不在报告中给出需要标注样本支撑的离线检索指标。当前评估重点放在数据覆盖规模、系统可用性和工程师试用反馈上，保留车型数量、BOM 数据条数、点云文件数量和平均响应耗时四项指标。这样既能说明系统已具备平台化使用基础，也避免用未验证的标注评测数据支撑结论。

平均响应耗时用于衡量服务端检索链路的基础性能：

$$
T_{avg}=\frac{1}{N}\sum_{i=1}^{N}t_i
$$

其中，$N$ 表示统计周期内的检索请求数量，$t_i$ 表示第 $i$ 次请求从服务接收到返回结果的耗时。该指标可以从服务日志中直接统计，适合现阶段作为网页端接入前的基础性能口径。

#### 2.4.1 评估结果与测试口径

以下数据为报告示例填充值，数值参考中等规模竞品数据平台的常见数据量级和内部检索服务体验目标设置。正式对外评审或上线验收前，应替换为当前数据库与服务日志的实际统计结果。

| 指标项 | 示例结果 | 说明 |
| --- | ---: | --- |
| 车型数量 | 12 款 | 覆盖轿车、SUV、MPV 等常见竞品类型 |
| BOM 数据条数 | 368,000 行 | 指完成解析、字段归一和入库后的有效 BOM 记录 |
| 点云文件数量 | 42,000 个 | 按车型目录和零件文件名扫描统计 |
| 平均响应耗时 | 620 ms | 包含查询向量化、Qdrant 召回、DuckDB 补全和结果返回 |

在缺少人工标注集的阶段，检索效果主要通过业务人员试用反馈进行判断。工程师在典型查询中能够用零件名称、简称、材料工艺组合和车型条件快速找到可用结果，整体体感明显优于人工打开 BOM 表和逐级查找点云目录。虽然该结论不能替代严格的离线评测，但能够说明产品已具备内部试运行价值。

产品优点可以从五个方面陈述。第一，系统把原本分散的 BOM 表和点云目录转化为统一资产，让竞品数据从“文件堆积”变成“可检索数据库”。第二，语义检索降低了对精确字段名和文件名的依赖，用户可以用工程习惯表达零件需求，再结合车型、材料、工艺等条件收敛结果。第三，点云文件与 BOM 记录在同一结果中呈现，减少了从表格到文件目录之间的人工跳转。第四，建库工具面向平台开发者和数据维护人员，支持自动监控和增量更新，适合持续接入新的竞品数据。第五，服务接口把检索、字段枚举和下载能力封装给网页端，最终用户只在竞品平台页面使用，不需要理解底层数据库、向量库和文件存储结构。

因此，现阶段产品价值不应表述为“已完成严格离线评测并达到某个检索指标”，而应表述为“完成了竞品 BOM 与点云数据的统一建库、语义检索和网页端服务化能力，已在人工试用中体现出较好的查询效率和结果可用性”。后续如果补充人工标注查询集，再增加召回质量、排序质量和错误类型等检索效果指标。
