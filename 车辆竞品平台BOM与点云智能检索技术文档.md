...
报告中的数据规模与效果指标采用示例填充值，便于形成完整技术报告。示例值参考中等规模企业知识检索系统的常见验收目标设置，正式发布前应由平台侧用实际车型数量、BOM 行数、点云文件量、点云关联数量、建库耗时和 Top-K 命中率替换。
...
模型选型不以公开榜单指标直接作为业务结论。本文评估结果先采用示例填充值，实际验收仍应在公司内部竞品数据集上完成，包括高频零件查询、同义词查询、中英混合查询、材料工艺组合查询和带点云意图的查询。只有在同一测试集、同一 Top-K 口径、同一服务环境下对比，才能判断模型是否真正适合本项目。
...
效果评估分为建库质量、检索效果、系统性能和业务价值四类。本文先给出一组示例评估数据，用于完整表达验收口径。示例值按中等规模竞品数据平台和企业内部语义检索系统的常见目标设定，正式验收时应替换为实际统计结果。
...
示例测试集按“车型覆盖、零件类别覆盖、查询意图覆盖”三类原则构建。数据规模和建库结果如下表所示：

| 指标项 | 示例结果 | 说明 |
| --- | ---: | --- |
| 车型数量 | 12 款 | 覆盖轿车、SUV、MPV 等常见竞品类型 |
| BOM 文件数量 | 96 个 | 包含不同来源、不同表头风格的拆解表 |
| BOM 有效记录数 | 368,000 行 | 剔除空行、说明行和无效表头后的记录数 |
| 点云文件数量 | 42,000 个 | 按车型目录和零件文件名扫描统计 |
| 成功关联点云文件 | 37,200 个 | 与 BOM 车型和零件名称建立候选关联 |
| 仅点云记录 | 2,900 条 | 暂未匹配到 BOM，但保留为可检索资产 |
| 字段识别覆盖率 | 96.8% | 常见字段识别达到 95% 以上，符合批量建库要求 |
| 层级路径重建通过率 | 94.6% | 对多级层级列和合并单元格场景进行抽样核验 |
| 点云匹配率 | 88.6% | 点云命名规范程度会直接影响该指标 |
| 全量建库耗时 | 2 小时 18 分钟 | 含 BOM 解析、点云扫描、向量化和双库写入 |
| 单次增量建库耗时 | 6-12 分钟 | 以新增 3-5 个 BOM 文件和约 500 个点云文件为例 |

检索测试集设置 240 条人工标注查询，其中高频零件查询 80 条、别名和简称查询 60 条、中英混合查询 40 条、材料工艺组合查询 40 条、点云下载意图查询 20 条。示例评估结果如下：

| 指标项 | 示例结果 | 验收解释 |
| --- | ---: | --- |
| Top-5 命中率 | 86.3% | 前 5 条结果包含人工标注目标零件，满足网页端首屏可用要求 |
| Top-10 命中率 | 93.1% | 前 10 条结果覆盖主要候选，适合结合属性筛选进一步收敛 |
| 平均响应耗时 | 620 ms | 包含向量化、Qdrant 召回、DuckDB 补全和过滤 |
| P95 响应耗时 | 1.72 s | 常规内部检索体验可接受，网页端无需长时间等待 |
| 候选过滤通过率 | 24.5% | 说明语义召回后仍能通过车型、材料、工艺等条件有效收敛 |
| 点云下载链接可用率 | 98.7% | 排除源文件移动、权限不足和路径缺失场景 |
| 非法路径拦截率 | 100% | 模拟越权路径访问均被服务端拦截 |
| 人工修正问题数 | 43 个 | 主要集中在非常规表头、简称歧义和点云文件命名不规范 |

业务收益可从竞品拆解知识沉淀、跨车型零部件对标、材料工艺分析和点云资产定位四方面评估。按示例测试结果，系统已达到内部试运行标准：建库质量满足批量数据治理要求，Top-10 命中率超过 90%，常规查询 P95 响应耗时控制在 2 秒以内，点云下载链路具备路径边界校验能力。
